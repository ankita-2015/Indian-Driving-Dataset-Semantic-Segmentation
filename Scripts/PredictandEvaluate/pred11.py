...
#         input_mask = tf.image.flip_left_right(input_mask)
    ...
    input_mask = tf.reshape(input_mask, (IMG_SIZE[0], IMG_SIZE[1], 3))
    return input_image, input_mask
...
def make_predictions(model, csv_path, test_images_folder_path, test_mask_folder_path):    
    ...
    for j in tqdm(range(0,len(test_x),BATCH_SIZE)):
        ...
        for i in range(BATCH_SIZE):
            ...
            y_predi = tf.image.resize(y_pred[i],IMG_SIZE ,method='nearest')
            y_predi = np.squeeze(np.argmax(y_predi, axis=-1))            
...
use_org_img_size = sys.argv[3]
model = load_model(f'../../RESULTS/{foldername}/ckpt_path/{epoch_num}.h5',
...
path = f"../../RESULTS/{foldername}/output_result_CS"
csv_path = f"../../RESULTS/{foldername}/TestResult_{epoch_num}_CS.csv"

IDD_size = (1080,1920)
BDD_size = (720,1280)
CS_size = (1024,2048)

if use_org_img_size=="True":
    IMG_SIZE = CS_size
else:
    IMG_SIZE = (512,512)
...
test_images_folder_path = "../../Dataset/CS/3classes/Test/images/*"
test_mask_folder_path="../../Dataset/CS/3classes/Test/masks/*"
